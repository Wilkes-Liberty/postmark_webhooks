...
Each cron run deletes 250 expired event rows per configured batch (default one
batch). Durable suppression is not deleted. Backlogs can remain older than the
configured retention age if cron cannot keep up; raise `event_retention_batches`
or run `drush postmark-webhooks:retention-drain`. Disabling suppression does
not delete evidence.
