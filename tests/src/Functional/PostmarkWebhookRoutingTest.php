...
class PostmarkWebhookRoutingTest extends BrowserTestBase {
  ...
  public function testWebhookHttpAuthentication(): void {
    ...
    $contents = file_get_contents($fixtures . 'bounce.json');
    $this->assertIsString($contents, 'The bounce fixture can be read.');
    $options = [
      'http_errors' => FALSE,
    ...
    foreach (['spam-complaint.json', 'subscription-change.json'] as $fixture) {
      $contents = file_get_contents($fixtures . $fixture);
      $this->assertIsString($contents, sprintf('The %s fixture can be read.', $fixture));
      $options['json'] = json_decode($contents, TRUE, 512, JSON_THROW_ON_ERROR);
      $this->assertSame(200, $client->post($url, $options)->getStatusCode());
      $this->assertSame(200, $client->post($url, $options)->getStatusCode());
    }
